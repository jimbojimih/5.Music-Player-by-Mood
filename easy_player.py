...
class PlayListCreator(QThread):
    ...
    def create_mood_playlists_id_dict(self):
        ...
        for mood, params in self.play_lists_json.items():
            self.play_lists_json[mood] = self.ytmusic.get_mood_playlists(
                    params)        
    ...
    def create_mood_list_of_video_id_dict(self):
        ...
        for mood, params in self.play_lists_json.items():
            ...
            for param in params:
                ...
                tracks_from_playlist_raw = self.ytmusic.get_playlist(
                        param)['tracks']
...
class PlaybackControl():
    ...
    def set_mood(self, mood): 
        ...
        self.playlist_append(current_playlist)
        self.player.playlist_pos = pos_track

    def update_last_tracks_played(self):
        '''Update in GUI'''
        self.last_tracks[self.mood] = [self.player.playlist_pos, 
                                       self.numb_playlist]
    ...
    def turn_on_the_next_playlist(self):
        ...
        if self.numb_playlist < len_playlist_of_mood:
            ...
            current_playlist = self.play_lists_json[
                    self.mood][self.numb_playlist]
    ...
    def turn_on_the_previous_playlist(self):
        if self.numb_playlist > 0:
            ...
            current_playlist = self.play_lists_json[self.mood][
                    self.numb_playlist]
    ...
    def playlist_append(self, current_playlist):
    ...
    def reset(self):
        self.last_tracks = {
                "Chill": [0, 0], "Commute": [0, 0], "Energy Boosters": [0, 0],
                "Feel Good": [0, 0], "Focus": [0, 0], "Party": [0, 0], 
                "Romance": [0, 0], "Sleep": [0, 0], "Workout": [0, 0]}
...
class MainWindow(QWidget):
    ...
    def init_ui(self):
        ...
        self.file_menu.addAction('Reset the save of lasts tracks played', 
                                  lambda: self.playback_control.reset())
        ...
        self.button_return.clicked.connect(
                lambda: self.playback_control.turn_on_the_previous_playlist())

        self.next_playlist = QPushButton("-->") 
        self.next_playlist.setFixedSize(85, 30) 
        self.next_playlist.clicked.connect(
                lambda: self.playback_control.turn_on_the_next_playlist())
        ...
        layout3.addStretch(1)                            
        layout3.addWidget(self.link) 

        self.setLayout(layout3)

        self.timer = QTimer()
        self.timer.timeout.connect(self.set_link)
        self.timer.timeout.connect(
                lambda: self.playback_control.update_last_tracks_played())
        self.timer.timeout.connect(
                lambda: self.playback_control.check_the_last_track())
    ...
    def set_link(self):
        ...
        if self.playback_control.get_video_id() is not None:
            ...
            url = ("<a href=\"{url_text}\"> <font color=black>"
                   "Open on YouTube</a>").format(url_text=url_text)
            self.link.setText(url)
        else:
            self.link.setText('wait')
    ...
    def show_about_app_window(self):
        ...
        text = ('A simple music player to listen to music in the background,' 
                'depending on your mood. Work is allowed on the playlists ' 
                'of YouTube Music. If the music is not suitable for you, ' 
                ...
                'музыка не подходит для Вас, вы можете переключить '
    ...
    def update_play_list(self):  
        ...
        text_finish_message = (
                'Playlists have been updated. You can reset the saving of the '
                'last played tracks. Playback will start with the updated '
                'playlists.')

        #multithreading
        self.update_play_list.finished.connect(
                lambda: QMessageBox.information(
                    self, text_title_finish_message, text_finish_message))
        self.update_play_list.start()

        text_title_warning_message = 'Update'
        text_warning_message = (
                'Please wait about 10 minutes. You can continue to use the '
                'player. The update takes place in the background. When the '
                'update is completed, you will see a notification.') 
        QMessageBox.warning(self, text_title_warning_message, 
                text_warning_message)
